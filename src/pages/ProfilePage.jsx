...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'CLUB') {
        await clubService.updateClubProfile(formData);
      } else {
        await authService.updateProfile(formData);
      }
      
      // Si on arrive ici, la mise à jour a réussi. On affiche le succès.
      setSuccess('Profil mis à jour avec succès !');

      // Étape 2: Essayer de rafraîchir les données de l'utilisateur en arrière-plan.
      // Si ça échoue, ce n'est pas grave, on ne montre pas d'erreur au-dessus du succès.
      try {
        await fetchUser();
      } catch (fetchError) {
